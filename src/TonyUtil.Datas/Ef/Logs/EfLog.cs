    ...
    public class EfLog : ILogger {
        ...
        public static string GetSql( string sql, string sqlParams ) {
            ...
            foreach ( var parameter in parameters )
                sql = TonyUtil.Helpers.Regex.Replace( sql, $@"{parameter.Key}\b", parameter.Value );
            return sql;
        }
        ...
        public static IDictionary<string, string> GetSqlParameters( string sqlParams ) {
            ...
            var parameters = TonyUtil.Helpers.Regex.Split( sqlParams, pattern );
            foreach( var parameter in parameters )
        ...
        private static string GetParamName( string sqlParams ) {
            string pattern = $@"([@].*?)\d+=";
            return TonyUtil.Helpers.Regex.GetValue( sqlParams, pattern, "$1" );
        }
        ...
        private static void AddParameter( Dictionary<string, string> result, string parameter, string paramName ) {
            ...
            var values = TonyUtil.Helpers.Regex.GetValues( parameter, pattern, new[] { "$1", "$2", "$3" } ).Select( t => t.Value ).ToList();
            if ( values.Count != 3 )
                return;
