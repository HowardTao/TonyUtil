...
namespace TonyUtil.Ui.TagHelpers
{
    /// <summary>
    /// TagHelperBase
    /// </summary>
    public abstract class TagHelperBase:TagHelper
    {
        ...
        {
            if (WriteLog) WriteTraceLog(render, "渲染TagHelper组件");
        }
        ...
        protected void WriteTraceLog(IRender render, string caption)
        {
            ...
            log.Class(GetType().FullName).Caption(caption).Content(render.ToString()).Trace();
        }
