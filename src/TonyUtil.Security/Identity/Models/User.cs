...
using TonyUtil.Security.Encryptors;
using TonyUtil.Validations;
using ClaimTypes = TonyUtil.Security.Claims.ClaimTypes;

namespace TonyUtil.Security.Identity.Models {
    /// <summary>
    /// 用户
    /// </summary>
    public partial class User<TUser, TKey> {
        ...
        public override ValidationResultCollection Validate() {
            if( UserName.IsEmpty() )
                throw new Warning( SecurityResource.UserNameIsEmpty );
        ...
        public virtual void AddUserClaims() {
            AddClaim( ClaimTypes.Mobile, PhoneNumber );
