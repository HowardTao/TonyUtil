...
namespace TonyUtil.Ui.Extensions {
    ...
    public static partial class Extensions {
        ...
        public static T GetValueFromAttributes<T>( this TagHelperContext context, string key ) {
            ...
            if( exists == false )  return default( T );
            if( !( value is TagHelperAttribute tagHelperAttribute ) )  return default( T );
            return Helpers.Convert.To<T>( tagHelperAttribute?.Value );
        }
        ...
        public static T GetValueFromItems<T>( this TagHelperContext context, object key ) {
            ...
            if( exists == false )  return default( T );
            if( !( value is TagHelperAttribute tagHelperAttribute ) ) return Helpers.Convert.To<T>( value ); ;
            return Helpers.Convert.To<T>( tagHelperAttribute?.Value );
        }
        ...
        public static void SetValueToItems( this TagHelperContext context, object key,object value ) {
            if ( context.Items.ContainsKey( key ) ) return;
