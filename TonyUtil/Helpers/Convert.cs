   ...
   public static class Convert
    {
        ...
        /// <summary>
        /// 转换为32位浮点型,并按指定小数位舍入，温馨提示：4舍6入5成双
        /// </summary>
        /// <param name="input">输入值</param>
        ...
        /// <summary>
        /// 转换为32位可空浮点型,并按指定小数位舍入，温馨提示：4舍6入5成双
        /// </summary>
        /// <param name="input">输入值</param>
        /// <param name="digits">小数位数</param>
        /// <returns></returns>
        ...
        public static T To<T>(object input)
        {
            ...
            if (input is string && string.IsNullOrWhiteSpace(input.ToString())) return default(T);
            var type = Common.GetType<T>();
            try
            {
                if (type.Name.ToLower() == "string") return (T) (object) input.ToString();
                if (type.Name.ToLower() == "guid") return (T) (object) new Guid(input.ToString());
                if (type.IsEnum) return Enum.Parse<T>(input);
                if (input is IConvertible) return (T) System.Convert.ChangeType(input, type);
                return (T) input;
            }
            catch
            {
